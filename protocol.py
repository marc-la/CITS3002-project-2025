...
def send_packets(message: str, connfile, debug=False):
    ...
    while len(acks) < len(packets):
        ...
        try:
            # Read 5 bytes for the ACK/NACK header
            ack_data = connfile.read(HEADER_SIZE)
            if not ack_data or len(ack_data) < 5:
                continue
            ack_seq, ack_type, _ = struct.unpack('!H B H', ack_data)
            ...
            if debug:
                print("Timeout or error, resending unacknowledged packets...")
            
    # Send final acknowledgment to indicate all packets were received
    final_ack = build_packet(seq + 1, PACKET_TYPE_ACK, b'')
    connfile.write(final_ack)
    connfile.flush()
    if debug:
        print("[send] Sent final acknowledgment to server.")

def receive_packets(connfile, debug=False) -> str:
    received_packets = {}
    expected_seq = 0

    while True:
        try:
            packet = connfile.read(1024)
            if not packet or len(packet) < 5:
                if debug:
                    print("[receive] Connection closed by peer or incomplete packet.")
                break
            ...
            if pkt_type == PACKET_TYPE_DATA:
                ...
                connfile.write(build_packet(seq, PACKET_TYPE_ACK, b''))
                connfile.flush()
                if debug:
...
def test_protocol():
    ...
    def run_server():
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            with conn:
                ...
                connfile = conn.makefile('rwb', buffering=0)
                received = receive_packets(connfile)
    ...
    def run_client():
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            connfile = s.makefile('rwb', buffering=0)
            send_packets(test_message, connfile, debug=True)
